...
| `shallow_water` | 44.4 vs 4.9 | 27.1 vs 12.9 | seams badly |
| `river` | 122.1 vs 15.3 | 90.6 vs 20.2 | the worst - it is one diagonal vignette |

So `river`, `shallow_water`, `mountain` and `forest` are where the work
actually is. `plains` and `reef` already wrap; `reef` still wants redrawing,
but for its colour, not its edges.
