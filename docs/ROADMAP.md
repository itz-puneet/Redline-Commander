...
- [x] Road and shoreline neighbour variants: all 20 the mechanism above
      ...
      `road.png`.
- [ ] Seamless terrain tiling. Water is done: `shallow_water` and `river`
      are generated by `art/png/make_water_tiles.py` from waves whose
      periods divide the tile, so they wrap exactly - `river` in particular
      was the worst tile in the set, one diagonal vignette that turned a
      river into a checkerboard of disconnected segments. `forest`,
      `mountain` and `road` are still illustrations and still repeat
      visibly; `art/png/check_tile.py` scores them and
      `docs/ASSET_PROMPTS.md` carries the prompts. Two tiles are also wrong
      for where they are used: the reef is a mid blue sitting in deep water,
      and every building sits on a dark card rather than on ground.
